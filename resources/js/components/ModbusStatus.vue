<template>
    <div class="modal modal-blur bg-white-overlay fade show" id="modbus_modal" tabindex="-1" style="display: block;" aria-modal="true" role="dialog">
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                <button type="button" @click.prevent="$emit('close_modbus')" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                <div class="modal-status bg-danger"></div>
                <div class="modal-body text-center py-4">
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon mb-2 text-danger icon-lg" aria-hidden="true" role="img" width="24" height="24" preserveAspectRatio="xMidYMid meet" viewBox="0 0 36 36">
                    ...
                    <h3>Nema Modbus komunikacije</h3>
                    <div class="text-muted">Preko modbus komunikacije dobijamo podatke o protocima bagera. Trudićemo se da brzo rešimo kvar.</div>
                </div>
                <div class="modal-footer">
                    <div class="w-100">
                        <div class="row">
                            <a href="#" @click.prevent="$emit('close_modbus')" class="btn btn-danger w-100" data-bs-dismiss="modal">
                                Ok
                            </a>
                        </div>
                    </div>
